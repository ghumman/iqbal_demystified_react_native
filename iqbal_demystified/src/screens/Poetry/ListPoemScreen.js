...
class ListPoemScreen extends React.Component {
  ...
  starToggling = (poem) => {
    ...
    this.readBookmarks().then((result) => {
      ...
      if (result.includes(poem.id)) {
        ...
        var path = `${RNFS.DocumentDirectoryPath}/bookmarked-poems.yaml`;
        const sherNumberComma = `${poem.id}@${poem.textUrdu}@${poem.textEnglish}@`;
  ...
  componentDidMount() {
    try {
      ...
      const bookName = this.props.navigation.getParam('detailBook');
  ...
  getPoemList(listId) {
    ...
    const that = this;
    StaticContentService.getPoemList(listId).then((response) => {
      ...
      that.readBookmarks().then((result) => {
        ...
        for (let i = 0; i < yamlObject.sections.length; i++) {
          try {
            ...
            if (yamlObject.sections[i].poems[0].poemName[0]) {
              for (let j = 0; j < yamlObject.sections[i].poems.length; j++) {
